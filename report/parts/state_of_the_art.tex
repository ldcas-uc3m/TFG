...
This chapter presents the current stage of the technologies related to our application, specifically the different kinds of simulators that are currently present in the market (\sectionref{assembly-simulators}), both specific to one architecture (\subsectionref{specific-assembly-simulators}) and architecture-agnostic simulators (\subsectionref{generic-assembly-simulators}).\\
Finally, it includes a comparison between all the mentioned simulators and our proposed application (\sectionref{comparison}).
...
In Table \ref{tab:comparison}, we show a comparison of the different features of the current assembly simulators and our proposal.  % TODO: explain characteristics
