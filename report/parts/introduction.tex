...
This first chapter briefly presents the project: why it came to be (\secref{motivation}), the objectives it strives to fulfill (\secref{objectives}), and a description of the overall document structure (\secref{structure}).
...
For all these reasons, amongst others, learning how \glspl{assembly} work is required for any \gls{computer science} student, or any \glsdisp{software engineering}{software engineer}, given that it offers a better understanding of the underlying architecture, therefore enabling them to more efficiently use that architecture, but also enabling them to better express those abstract ideas in terms that the \gls{processor} will understand.
...
\begin{itemize}
  ...
  \item \textbf{O2:} Define a simple \gls{programming language} that allows to \glsdisp{programming}{program} the different components of the simulated \gls{computer}.
...
\begin{itemize}
  \item \chapref{introduction}, briefly presents the project, its motivations and objectives, and a description of the contents of the document.
  \item \chapref{state-of-the-art}, includes a description of the current stage of \gls{assembly} simulators, providing examples and comparing them with our proposal.
  \item \chapref{analysis}, presents the project and establishes its requirements and use cases.
  \item \chapref{design}, details the design process and the system's architecture, including all of its components.
  \item \chapref{implementation}, includes the implementation details of the main parts of the developed software, and the necessary steps for its deployment.
  \item \chapref{validation}, details the verification and validation processes of the system and its objectives, including test cases.
  \item \chapref{planning}, presents the concepts related to the followed planning, and  breaks down all project costs. It also discusses the regulatory framework and socio-economic environment that applies to the project.
  \item \chapref{conclusions}, highlights the contributions of the project, discusses the overall conclusions, and presents what future work could be done in order to improve the system.
  \item \appref{manual}, provides the user manual for the software.
\end{itemize}