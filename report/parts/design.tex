...
% To provide a more detailed description how the different components interact between them, several UML sequence diagrams were defined \parencite{Cook2017}:
% \begin{itemize}
%   \item a
% \end{itemize}
...
The LUISP-DA \gls{interpreter} is implemented in the \reqref{Control Unit} component. As stated in \subsecref{interpreter-study}, the architecture of the \gls{interpreter} will be based on the Make-A-Lisp \parencite{mal} architecture. Nevertheless, as it is only needed to model a simple \glsdisp{assembly}{assembly-like language}, a subset of the original architecture will be used, modified in order to fit the project's purposes.
...
\begin{itemize}
  ...
  \item \textbf{\glsdisp{text memory}{Text} segment:} The compiler will go instruction by instruction, translating and temporarily saving them to LUISP-DA with the locations of tags, if any. Afterwards, the compiler will translate the tags in the instructions by the addresses they represent, and load the finished instructions in the \gls{text memory}.
\end{itemize}



\subsection{ISA Definition}\label{subsec:isa-design}
As stated in Subsection \subsecref{isa-format-study}, the \gls{ISA} will be defined in a \gls{json} format, which will also serve as the configuration file for the simulator. For the configuration, several things need to be defined:
\begin{itemize}
